...
const App = () => {
  ...
  const generateIdentity = async () => {
    ...
    const prompt = `
      ...
      {
        "strategy": "تحليل استراتيجي عميق يربط الأهداف بالهوية",
        "general_summary": "ملخص عام شامل يدمج الرؤية الاستراتيجية بالهوية المقترحة بأسلوب ملهم واحترافي",
        ...
        "professional_ai_prompt": "A professional, high-end image generation prompt in English for Midjourney/DALL-E. It must describe a masterpiece logo for '${answers.brand_name}' incorporating ${answers.brand_vibe} style, ${answers.shapes_concept} elements, and ${answers.color_preference} palette. Emphasis on: minimal, flat design, vector, 8k resolution, symmetrical, golden ratio, professional studio lighting. If the brand name involves Arabic, specify elegant modern Arabic calligraphy."
      }
    `;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans p-4 md:p-8 flex flex-col items-center overflow-x-hidden" dir="rtl">
          ...
          <div className="p-8 md:p-12 animate-in zoom-in-95 duration-500">
            ...
            {result && (
              <div className="space-y-10">
                {/* 0. General Summary (New) */}
                <section className="bg-indigo-50 p-6 rounded-3xl border border-indigo-100">
                  <h3 className="flex items-center gap-2 text-indigo-800 font-black text-xl mb-4">
                    <FileText className="w-6 h-6" /> الملخص العام والاستراتيجي
                  </h3>
                ...
                {/* 4. Professional AI Prompt (New) */}
                <section className="bg-slate-900 text-white p-8 rounded-[2rem] border-t-4 border-indigo-500 shadow-xl">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="flex items-center gap-2 font-black text-xl text-indigo-300">
                      <Sparkles className="w-6 h-6" /> البرومبت الاحترافي لتوليد الشعار (AI)
                    </h3>
                    <button 
                      onClick={() => copyToClipboard(result.professional_ai_prompt)}
                  ...
                  <div className="bg-slate-800/50 p-6 rounded-2xl border border-slate-700/50">
                    <p className="text-indigo-100 leading-relaxed font-mono text-sm text-left" dir="ltr">
                      {result.professional_ai_prompt}
                    </p>
                  </div>
                  <p className="mt-4 text-slate-400 text-xs text-center italic">
                    * انسخ النص أعلاه واستخدمه في Midjourney أو DALL-E للحصول على أفضل نتيجة بصرية.
                  </p>
                </section>
      ...
      <footer className="mt-12 pb-8 text-center w-full max-w-4xl">
        ...
        <p className="text-slate-400 text-sm tracking-[0.2em] font-medium uppercase mb-2">
            Professional Identity Consultant v2.5
        </p>
        <p className="text-lg">
